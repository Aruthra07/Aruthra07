...
for Aruthra S M's GitHub Profile README.
"""
...
THEMES = {
    "dark": {
        ...
        "DIM": "#475569",
        "STROKE": "rgba(34,211,238,0.28)", "STROKE_HI": "rgba(34,211,238,0.55)",
...
def set_theme(name):
    ...
    for k, v in t.items():
        g[k] = v
    g["DONUT_COLORS"] = [t["VIOLET"], t["CYAN"], t["EMERALD"], "#6366F1", "#F59E0B", "#94A3B8"]
...
def card(p, x, y, idx):
    ...
    a(f'<rect width="{CARD_W}" height="{CARD_H}" rx="12" fill="{PANEL}" stroke="{STROKE}">'
      ...
      f'dur="4.5s" begin="{b+idx*0.5:.2f}s" repeatCount="indefinite"/></rect>')
    ...
    float_anim = (f'<animateTransform attributeName="transform" type="translate" '
                  f'values="0 0; 0 -2; 0 0" dur="4.5s" begin="{b+idx*0.4:.2f}s" '
                  f'repeatCount="indefinite" calcMode="spline" keyTimes="0;0.5;1" '
                  f'keySplines="0.4 0 0.6 1;0.4 0 0.6 1"/>')
    a(f'<g>{float_anim}<rect x="16" y="44" width="40" height="40" rx="9" fill="{VIOLET2}" opacity="0.85"/>'
    ...
    a(f'<text x="68" y="61" font-size="16" font-weight="700" fill="{TEXT}">{name}'
      f'<tspan fill="{CYAN}">_<animate attributeName="opacity" values="1;0;1" dur="1.2s" '
      f'begin="{b+0.3:.2f}s" repeatCount="indefinite"/></tspan></text>')
    ...
    a(f'<text x="68" y="155" font-size="11" fill="{MUTED}">'
      ...
      f'<tspan fill="{DIM}" dx="14">status: active</tspan></text>')

    # donut chart fallback / display
    langs = p.get("languages") or {"Python": 60, "Data": 25, "AI": 15}
...
def build(projects, theme="dark"):
    ...
    a(f'<defs><linearGradient id="{gid}" x1="0" y1="0" x2="1" y2="0">'
      ...
      '</linearGradient></defs>')
    a(f'<text x="{MARGIN+2}" y="18" font-size="11" letter-spacing="2" fill="{CYAN}">ARUTHRA.FEATURED_PROJECTS</text>')
    a(f'<text x="{MARGIN+210}" y="18" font-size="10" fill="{DIM}">./build_portfolio.sh --featured</text>')
...
if __name__ == "__main__":
    src = sys.argv[1] if len(sys.argv) > 1 else "projects.json"
